...
- Laplace feature
  ...
  - `GET /api/applications/laplace/simulate` uses `scipy.integrate.solve_ivp` on a forced spring-mass-damper system.
...
- Fourier feature
  ...
  - `GET /api/applications/fourier/signal` generates a truncated Fourier approximation for `f(x)=x` over `[-pi, pi]`.
...
- `/api/applications/laplace/simulate`
  ...
  - Returns equal-length arrays for `t`, `displacement`, `velocity`, and `forcing`.
...
- `/api/applications/fourier/signal`
  ...
  - Returns equal-length arrays for `x`, `signal`, and `approximation`, plus `terms_used`.
